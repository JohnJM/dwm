...
void arrange(Monitor *m) {
  ...
  for (Monitor *mon = (m ? m : mons); mon; mon = (m ? NULL : mon->next)) {
    ...
    /// all layouts need added here if they want to not be consumed by
    /// autoswitch
    if (!mon->manualswitch && mon->lt[mon->sellt]->arrange != tcl &&
        mon->lt[mon->sellt]->arrange != spiral &&
        mon->lt[mon->sellt]->arrange != dwindle &&
        mon->lt[mon->sellt]->arrange != NULL) {
      if (n >= monoclemode && mon->lt[mon->sellt]->arrange != tile) {
        setlayout(&(Arg){.v = &layouts[1]});
      } else if (n < monoclemode && mon->lt[mon->sellt]->arrange == tile) {
...
void tile(Monitor *m) {
  ...
  for (i = my = ty = 0, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      resize(c, m->wx, m->wy + my, mw - (2 * c->bw) + (gappx), h - (2 * c->bw),
             0);
      if (my + HEIGHT(c) < m->wh)
        my += HEIGHT(c);
    } else {
...
int visibleclientcount(Monitor *m) {
